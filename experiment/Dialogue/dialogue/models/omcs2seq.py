...
class OMCSEncoderDecoder(BaseDeepModel):
    def __init__(self, loss_fn, opt):
        super(OMCSEncoderDecoder, self).__init__()
        ...
        self.event_id_embedding = nn.Embedding(opt.model.omcs_vocab_size, rnn_hidden_size // 4)
        self.event_embedding = nn.Embedding(opt.model.omcs_event_vocab_size, rnn_hidden_size // 4)
        self.relation_embedding = nn.Embedding(opt.model.omcs_relation_vocab_size, rnn_hidden_size // 4)
    ...
    def run_batch(self, batch):
        ...
        dec_probs = self.forward(
            ...
            event_id_inputs=batch.omcs_id_inps,
            event_triple_inputs=batch.omcs_triple_inps,
            event_lens=batch.omcs_lens)
