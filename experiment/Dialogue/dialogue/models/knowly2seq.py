...
class KnowlyEncoderDecoder(BaseDeepModel):
    def __init__(self, loss_fn, opt):
        super(KnowlyEncoderDecoder, self).__init__()
        ...
        self.event_id_embedding = nn.Embedding(opt.model.knowly_vocab_size, rnn_hidden_size // 4)
        self.event_embedding = nn.Embedding(opt.model.knowly_event_vocab_size, rnn_hidden_size // 4)
        self.relation_embedding = nn.Embedding(opt.model.knowly_relation_vocab_size, rnn_hidden_size // 4)
    ...
    def run_batch(self, batch):
        ...
        dec_probs = self.forward(
            ...
            event_id_inputs=batch.knowly_id_inps,
            event_triple_inputs=batch.knowly_triple_inps,
            event_lens=batch.knowly_lens)
